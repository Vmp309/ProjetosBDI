...
class MenuBuscar:
    ...
    def exibir_menu(self, tipo ):
            ...
            if (opcao == 1): # Pesquisa o livro pelo nome
                ...
                livro = self.consultarPreco(maior, menor)
                if (livro != None): # Se o livro não for None, retorna o id do livro
                    return livro
                else:
                    return
    ...
    def consultarNome(self , nome):
        ...
        if (self.tipo == "operação_vendedor"): # verifica se o vendedor que listar com qtd menor que 5
    ...
    def consultarPreco(self, maior, menor):
        if (self.tipo == "operação_vendedor"):
            print("1) Filtrar por quantidade menor que 5")
            print("2) Não Filtrar")
            filtrar = input("-> ")
        livros = None
        ...
        if (livros == None):
            ...
            return
        if (self.tipo == "operação_busca"): # Não é cliente nem vendedor
            print(livros)
            return 
        return livros
    ...
    def ConsultarMari(self):
        ...
        if (self.tipo == "operação_vendedor"): # Vendedor
            print("1) Filtrar por quantidade menor que 5")
            print("2) Não Filtrar")
            filtrar = input("-> ")
